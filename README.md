...
godot_console --headless --path . res://tools/smoke_test.tscn     # M2 冒烟测试 95 项（退出码 0 = 通过）
godot_console --headless --path . res://tools/scene_smoke.tscn    # 地图场景冒烟（退出码 0 = 通过）
```

当前可玩内容（M2 垂直切片）：标题界面 → 序章（穿越、双系觉醒、唐月教学战）→ 博城主城（重逢穆宁雪入队、天澜高中宇昂挑衅）→ 北门出城进灰雾林地（暗雷练级、明雷精英「独眼魔狼王」、黑教廷线索收尾）。剧情 NPC 实体出现在地图上（头顶「!」标记，按 E 交互），穆宁雪入队后在地图上跟随队伍，HUD 左下角显示当前主线目标。杂货商出售消耗品与装备（I/Tab 背包内使用/穿戴），战斗中可使用道具。占位音频：标题/城镇/林地/战斗四首 BGM 随场景切换，战斗与 UI 配音效。篝火处休息、修炼、突破、存档（单存档位，标题界面可继续旅程）。战败自动读回最近存档，无存档则重开新旅程。
...
│   ├── save_system.gd   #   SaveSystem：存档系统
│   ├── dialogue.gd      #   Dialogue：对话系统
│   └── audio_manager.gd #   Audio：音频管理（BGM/SFX）
...
- 输入映射已在 `project.godot` 中预定义：`WASD/方向键` 移动、`E` 交互、`空格/J` 攻击、`I/Tab` 背包、`Esc` 暂停、`Z` 战斗增幅
- **操作以键盘为主**：探索 WASD+E；战斗与菜单全程 方向键/WASD 导航 + 回车/E 确认 + Esc 返回（战斗中 Z 循环星辉增幅 0-3），鼠标仅作辅助
