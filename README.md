...
godot_console --headless --path . res://tools/smoke_test.tscn     # M2 冒烟测试 79 项（退出码 0 = 通过）
godot_console --headless --path . res://tools/scene_smoke.tscn    # 地图场景冒烟（退出码 0 = 通过）
```

当前可玩内容（M2 垂直切片）：标题界面 → 序章（穿越、双系觉醒、唐月教学战）→ 博城主城（重逢穆宁雪入队、天澜高中宇昂挑衅）→ 北门出城进灰雾林地（暗雷练级、明雷精英「独眼魔狼王」、黑教廷线索收尾）。剧情 NPC 实体出现在地图上（头顶「!」标记，按 E 交互），穆宁雪入队后在地图上跟随队伍，HUD 左下角显示当前主线目标。占位音频：标题/城镇/林地/战斗四首 BGM 随场景切换，战斗与 UI 配音效。篝火处休息、修炼、突破、存档（单存档位，标题界面可继续旅程）。战败自动读回最近存档，无存档则重开新旅程。
